 ...
 * 小小的渣渣，大大的挑担！
 * 
 * Code&Software © EternalIllusion A.K.A. EterIll, 2025
 * 
 * AP IP 192.168.69.1
 * 
 * @package TD_ESP32
 * @author EterIll
 * @author MyZhaZha
 * @version R_1.2.8b(RELEASE)
 * @description Bugfix:Cannot hide AP wifi.
 * @sponsor 一袋星光
 */
...
// 设置创建热点的密码，确保密码长度不少于8个字符 最好别连，用IP地址。
const char* ap_password = "13572468";

// 默认连接的WIFI配置
String default_ssid = "PandoraBox";
String default_password = "myzhazha.top";

//启用更详尽的串口日志
const bool detailedlog = false;

//启用AP网络,用于联网后显示IP地址
const bool enableap = true;
...
void setup() {
  ...
  //配网部分
  PrefSSID = default_ssid;
  PrefPassword = default_password;

  //首次使用自动进入配网模式,读取NVS存储空间内的ssid、password
  Preferences prefs;
  prefs.begin("wifi");
  if (prefs.isKey("ssid")) {                     //读取账号
    PrefSSID = prefs.getString("ssid", default_ssid);  //如果键值为空，返回"TD"
  }

  if (prefs.isKey("password")) {                         //读取密码
    PrefPassword = prefs.getString("password", default_password);  //如果键值为空，返回你渣姨的WIFI密码
  }
  bool needshowap = prefs.getBool("rap", false);
  prefs.putBool("rap", false);
